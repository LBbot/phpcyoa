...
if (isset($_SESSION["unconfirmed_email"]) && !empty($_SESSION["unconfirmed_email"])) {
    // empty bracket PASS: carry on to the bottom to just delete the session cookie
} elseif (session_cookie_check() === false) {
    ...
    if (isset($_COOKIE["et_cookie"]) && !empty($_COOKIE["et_cookie"])) {
        ...
        try {
            ...
            // make sure we get something back from couch or PHP will flip, looking for infinite arrays that don't exist
            if (sizeof($couch_rows_arr["rows"]) > 0) {
...
// note: We want to skip the et_cookie and just do this to get rid of a session cookie if user is in account activation
$_SESSION = array();
session_destroy();
